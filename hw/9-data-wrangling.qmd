...
> Note on using ChatGPT: You are free to use ChatGPT and other AI tools (and just good 'ol Googling and Stackoverflow, etc.) to answer these questions, but don't just blindly copy-paste code. If you do use ChatGPT, include the prompt(s) you used in a comment (using `#`). Did you have to change anything to your prompt to get better results? If so, write that down too in your code with a comment. Learn to use ChatGPT as a learning assistant - a tool to help you accomplish the task - rather than just a solutions manual. One version of using it makes you a better coder, the other robs you of that.
