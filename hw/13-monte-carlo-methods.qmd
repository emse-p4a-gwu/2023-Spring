...
### 3) Monte Carlo Full House [COLLABORATIVE, 10%]

Using the `sample()` function, conduct a monte carlo simulation to estimate the probability of drawing a ["Full House"](https://en.wikipedia.org/wiki/List_of_poker_hands#Full_house) poker hand from a standard 52-card deck.

### 4) Monte Carlo Right Triangles [COLLABORATIVE, 10%]

Using the `sample()` function, conduct a monte carlo simulation to estimate the probability of randomly picking three integer values between 1 and 1000 (inclusive) that can be used to form a right triangle. For example, drawing the points 3, 4, and 5 would work since `3^2 + 4^2 == 5^2`.

### 5) Monte Carlo Integration [COLLABORATIVE, 10%]
...
### 6) Your Own Monte Carlo Simulation [SOLO, 10%]
